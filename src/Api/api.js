...
export const flickerImage = (searchQuery) => {
    const FLICKER_API_ENDPOINT = `https://api.flickr.com/services/rest/?
    method=flickr.photos.search&text=${searchQuery}&api_key=${FLICKR_API_KEY}&format=json&nojsoncallback=1&per_page=10`;

    return fetch(FLICKER_API_ENDPOINT)
        .then(response => {
            return response.json();
        })
        .then(json => {
            json.photos.photo.map(({ farm, server, id, secret, title }) => ({
                id,
                title,
